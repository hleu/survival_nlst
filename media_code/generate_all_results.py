...
def run_surv(use_case, run_type, x_train, x_test, y_train, y_test, df_train, df_test):
    ...
    compare_td_aucs(y_train, y_test, cph_pred_test, rsf_pred_test, f"- {use_case}-{run_type}", f"{use_case}_{run_type}.png", reset=True)
...
def perform_all_test(use_case, x_train, x_test, y_train, y_test, df_train, df_test, feature_names, subset=[]):
    cph_model, rsf_model = run_surv(use_case, 'all', x_train, x_test, y_train, y_test, df_train, df_test)

    cph_subset = get_feature_importances_cph(cph_model, x_train, x_test, y_train, \
        df_train, df_test, feature_names, f'cph_{use_case}_all')

    rsf_subset = get_feature_importances_rsf(rsf_model, x_train, x_test, y_train, \
        feature_names, f'rsf_{use_case}_all')

    print(f"Most important CPH-{use_case} subset names:", cph_subset)
    print(f"Most important RSF-{use_case} subset names:", rsf_subset)

    f_tmp = open(subset_file, "a")
    ...
    if len(subset) > 0:
        ...
        _,_ = run_surv(use_case, 'subset', x_train_subset, x_test_subset, y_train, y_test, df_train, df_test)
...
def main():
    ...
    x_train = x_train.drop(columns=['ethnic=2.0']) 
    x_test = x_test.drop(columns=['ethnic=2.0']) 
    ...
    perform_all_test('clinical', x_train, x_test, y_train, y_test, df_train, df_test, clin_feature_names_new, clin_subset)
    ...
    ###################################################################
    x_train_rad, y_train, df_train = parse_radiomics(train_pos+val_pos, prsndf_rad)
    x_test_rad, y_test, df_test = parse_radiomics(test_pos, prsndf_rad)
    ...
    perform_all_test('radiomics1', x_train_rad1, x_test_rad1, y_train, y_test, df_train, df_test, rad1_feature_names, radiomics1_subset)
    ...
    perform_all_test('radiomics2', x_train_rad2, x_test_rad2, y_train, y_test, df_train, df_test, rad2_feature_names, radiomics2_subset)
    
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    print("\nRadiomics3:")
    radiomics3_subset = ['original_ngtdm_Busyness', 'original_firstorder_RootMeanSquared']
    perform_all_test('radiomics3', x_train_rad3, x_test_rad3, y_train, y_test, df_train, df_test, rad3_feature_names, [])
    ...
    perform_all_test('combination', x_train_comb, x_test_comb, y_train, y_test, df_train, df_test, comb_feature_names, [])
