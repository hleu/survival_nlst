...
def compare_td_aucs(y_train, y_test, prediction_cph_test, prediction_rsf_test, title_suffix, fig_suffix, label_suffix='', reset=False):
    if reset is True:
        plt.figure()
    ...
    cph_auc, cph_mean_auc = cumulative_dynamic_auc(
                y_train, y_test_mod, prediction_cph_test, va_times,
                )

    rsf_auc, rsf_mean_auc = cumulative_dynamic_auc(
                y_train, y_test_mod, prediction_rsf_test, va_times,
                )
...
def get_feature_importances_rsf(model, x_train, x_test, y_train,
    ...
    html = w.data
    with open('results/' + weight_suffix +'.html', 'w') as f:
        f.write(html)
...
def get_feature_importances_cph(model, x_train, x_test, y_train, df_train, df_test, feature_names, weight_suffix): 
    x_train_lifelines = x_train.join(df_train[['days_1stpos_death', 'death_cancer']])
    x_test_lifelines = x_test.join(df_test[['days_1stpos_death', 'death_cancer']])
    ...
    model_df.to_html('results/' + weight_suffix + '.html')
